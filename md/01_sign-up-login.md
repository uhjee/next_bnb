...
- state가 변경되면, 부모 컴포넌트가 업데이트 되면서 자식 컴포넌트들 또한 리렌더링

## 10.18 헤더 컴포넌트 최적화

### 현재 헤더 컴포넌트 렌더링에 영향을 끼치는 값

```typescript
const { openModal, ModalPotal, close,Modal } = useModal();
...
- 헤더의 인증 관련 코드들을 컴포넌트로 분리하여 해당 컴포넌트에서만 렌더링 되도록 처리
...
### 새로운 값을 만들어 props로 전달되는 부분 수정

componets/auth/SignUpModal.tsx

```tsx
          <Selector
            options={yearList}
            defaultValue="Year"
            disabledOptions={['Year']} // 현 컴포넌트에서  return 시 매번 새로 생성해 Selector 도 재렌더링
            value={birthYear}
            onChange={onChangeBirthYear}
            isValid={!!birthDay}
          />
