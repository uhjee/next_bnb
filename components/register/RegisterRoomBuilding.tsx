...
const RegisterRoomBuilding: FC<RegisterRoomBuildingProps> = () => {
  ...
  const onChangeLargeBuildingType = (event: ChangeEvent<HTMLSelectElement>) => {
    // redux 변경
    dispatch(registerRoomActions.setLargeBuildingType(event.target.value));
  };

  // 건물 change Handler
  const onChangeBuildingType = (event: ChangeEvent<HTMLSelectElement>) => {
    dispatch(registerRoomActions.setBuildingType(event.target.value));
  };

  // 숙소 유형 change handler
  const onChangeRoomtType = (value: string) => {
